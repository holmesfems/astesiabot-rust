...
pub async fn riseistages(
    ...
    let reply = match state
        .risei_calculator
        .stage_search(requested_server, &stage, &state.outer_source)
        .await
    {
        Err(msg) => EmbedReply::error(&msg),
        Ok((effective_server, stages)) => {
            let title = if effective_server == Server::Mainland && requested_server == Server::Global {
                "通常ステージ検索(大陸版)".to_string()
            } else {
                "通常ステージ検索".to_string()
            };
            let mut header = format!("検索内容 = {stage}");
            if effective_server == Server::Mainland && requested_server == Server::Global {
