...
│   │   └── skill_names.rs   … スキルID→表示名のみ（説明文/blackboard置換は未実装。
│   │                           skill_table.json をfetch）。SEED_PATH = data/seed/skill_names.json
...
- **operator_cost_calc はスキル説明文(SkillIdToName)を実装していない**: skillMasterCostの
  embedにPython版が付ける説明文(blackboard置換込みのテキスト生成)は意図的にスコープ外。
  `skill_names.rs`はスキル名の解決のみ行う。将来実装する場合は`skill_table.json`のblackboard
  置換ロジックを別途移植する必要がある。
...
- [ ] SkillIdToName（スキル説明文のblackboard置換）の移植。skillMasterCostのembedに
      説明文を出したい場合はここが必要
